...
## Align Image Filenames with Keypoints (time + master TimeFrames)

If your image filenames encode frame identity (for example `frame_10452.tif`) and your
keypoint table uses the same frame identity, do not rely on row order alone.

Use explicit TimeFrames so keypoints map to the filename-derived frame IDs.

### Step 1: Create `time` and `master` from image filenames

Create a sparse `time` TimeFrame using only values found in filenames, plus a dense
`master` TimeFrame covering the full range.
...
### Step 2: Load images and keypoints onto the same clock

Assign both the image stack and point data to `time` so keypoint frame IDs align to
filename-extracted IDs (not just row position).
...
::: {.callout-important}
## Frame Column Requirement
Point CSV loading expects an integer frame column. If your keypoint rows contain full
filenames (for example `frame_10452.tif`), extract the numeric ID first using the same
regex logic you used for the TimeFrame (`frame_(\d+)\.tif$`).
:::

::: {.callout-note}
## When to Use `master`
Use `time` for exact filename-ID mapping. Use `master` when you also need a dense,
continuous timeline (for example when combining imaging with other streams).
:::

### Complete Example: Image Stack + Keypoints with Filename IDs
...
[
  ...
  },
  {
    "data_type": "point",
    "name": "nose_tip",
    "filepath": "tracking_keypoints.csv",
    "format": "csv",
    "frame_column": 0,
    "x_column": 1,
    "y_column": 2,
...
- [Keypoint Video Labeling](keypoint_labeling.qmd) - Creating and exporting point tracks
- [TimeFrame](../concepts/timeframe.qmd) - Temporal coordinate systems and clock mapping
